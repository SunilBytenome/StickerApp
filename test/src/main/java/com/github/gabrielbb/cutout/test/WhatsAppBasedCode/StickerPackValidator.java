...
public class StickerPackValidator {
    ...
    private static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(StickerContentProvider.CONTENT_SCHEME).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY2).appendPath(StickerContentProvider.STICKERS_ASSET).appendPath("2").appendPath(stickerName).build();
    }
}
